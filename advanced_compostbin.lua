...
minetest.register_node("skytest:mulch_"..mat, {
...
minetest.register_node("skytest:machine", {
	description = "Advanced Compost Machine",
        _doc_items_usagehelp = "Use to turn leaves to mulch, mulch to dirt, clay or dyed mulch.",
	tiles = {
...
----------------------------------------------------------------------
		if resstack:get_name()=="skytest:mulch_tan" and
...
then 
	...
	if fields["makedirt"] then
		...
		if dirtstack1:get_name()=="skytest:mulch_"..mat and
		   dirtstack2:get_name()=="skytest:mulch_"..mat then
				make_dirt = "1"
		end
...
for i in ipairs (mulch_tab) do
	...
	local image = mulch_tab[i][3]
		...
		if deco == "skytest:mulch_"..mat then
...
minetest.register_craft({
		output = 'skytest:machine',
		recipe = {
			{'skytest:growth_crystal', 'skytest:infested_leaves', 'skytest:growth_crystal'},
			{'group:wood', 'default:apple', 'group:wood'},
			{'skytest:growth_crystal', "skytest:cobblegen", 'skytest:growth_crystal'},		
		},
})
